...
import { RSISystemUpdateMemberStatusHandler, jobInfo, main } from '../handlers/update-member-status-handler'
...
import mockRSIRequest from '../mocks/rsi-system/update-member-status/request'
import mockRSIResponse from '../mocks/rsi-system/update-member-status/response'
...
describe('[' + jobInfo.id + '] ' + jobInfo.name, () => {
  ...
  const mockJobContext = {
    functionName: 'update-rsi-member-status',
    localData: {
      jobInfo: jobInfo
    }
  }
  ...
  beforeAll(() => {
    ...
    helper._set_mock_rsi_credentials()
    helper._set_mock_rsi_access_token(mockRSIAccessToken)
  })
  ...
  beforeEach(() => {
    mockMPPRequestData = mockRSIRequest.updateMembersStatus(faker.random.number({ min: 1, max: 10 }))
  ...
  describe('All Test Cases', () => {
    each(testCases).test('%s - mpp adapter should return list of users with format', async (source, mockEvent) => {
      const handler = new RSISystemUpdateMemberStatusHandler(mockEvent, mockJobContext)
    ...
    each(testCases).test('%s - should have proper data and format for RSI custom api', async (source, mockEvent) => {
      mockRSIAxios.onPut(/\/member\/\d+\/activestatus/).reply(config => {
      ...
      await Promise.all(mockMPPRequestData.map(async (mockSampleData) => {
        const { status, data } = await handler.updateRSIMemberStatus(mockSampleData)
    ...
    each(testCases).test('%s - should be able to send email notifications for errors upon MPP query', async (source, mockEvent) => {
      ...
      const handler = new RSISystemUpdateMemberStatusHandler(localEvent, mockJobContext)
  ...
  describe('Test Case: aws.events', () => {
    test('should be able to execute all successfully from main', async () => {
      ...
      mockRSIAxios.onPut(/\/member\/\d+\/activestatus/).reply(200, mockRSIResponse.success)
    ...
    test('should be able to enqueue failed job data from main', async () => {
      ...
      mockRSIAxios.onPut(/\/member\/\d+\/activestatus/).reply(200, mockRSIResponse.fail)
      await main(mockCloudwatchScheduledEvent, mockJobContext, callback)
    })
    ...
    test('should be able to enqueue network errors from main', async () => {
      ...
      mockRSIAxios.onPut(/\/member\/\d+\/activestatus/).networkError()
      await main(mockCloudwatchScheduledEvent, mockJobContext, callback)
    })
    ...
    test('should be able to send email notifications for invalid client data from main', async () => {
      ...
      mockRSIAxios.onPut(/\/member\/\d+\/activestatus/).reply(400, {})
      await main(mockCloudwatchScheduledEvent, mockJobContext, callback)
    })
  })
  ...
  describe('Test Case: aws.lambda (Re-Invoke)', () => {
    ...
    test('should be able to re-enqueue partially failed job data from main', async () => {
      ...
      mockRSIAxios.onPut(/\/member\/\d+\/activestatus/).reply(() => {
